...
public class StudentUtil implements Serializable {
	...
	public static void setPaginationRecord(SessionInfo sessionInfo, Pagination pagination, List<DTOBase> academicProgramList) {
		for(DTOBase dto: pagination.getCurrentPageRecordList()) {
			...
			strBuff.append("	<img alt='image' class='img-circle' src='" + profilePict + "'>");
			strBuff.append("	<h3 class='m-b-xs'><strong>" + user.getName(false, true, false) + "</strong></h3>");
			strBuff.append("	<div class='font-bold'>Graphics designer</div>");
			strBuff.append("    <address class='m-t-md'>");
			strBuff.append("       	<strong>" + user.getCityPermanent().getName() + "</strong><br>");
			strBuff.append(		   	user.getStreetPermanent() + "<br>");
			strBuff.append(			user.getBarangayPermanent() + "<br>");
			strBuff.append("       	<i class=\"fa fa-mobile-phone\"></i>" + user.getCpNumber());
			strBuff.append("    </address>");
			strBuff.append("	<div class='contact-box-footer'>");
			strBuff.append(			pagination.getLinkButtonStr(sessionInfo, student.getId()).replace("~", ","));
			strBuff.append("	</div>");
			strBuff.append("</div>");
